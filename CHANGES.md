...
The lexer rule that makes generic type-argument syntax such as `List<String>` invisible to the parser matched far more than actual type arguments: starting at any `<`, it swallowed everything up to the next `>`, with no awareness of where it was or whether the content could plausibly be a type (#838). `x < y > y;` silently misparsed as a bogus declaration of a new `y`, discarding the old one; `x << b > b;` and `x = u < v ? -1 : u > v ? 1 : 0;` misparsed the same way, the first `<` swallowing everything up to the wrong `>`. The rule now also requires what follows a `<` to plausibly start a type name or wildcard (`A`-`Z`, `?`, `_`, or `$`, optionally after leading whitespace, matching this project's own convention that a variable name doesn't begin that way) before it will match, and excludes a few more characters arithmetic and ternary expressions use (`- + * / % ! ~ :`) but a type argument list never does. Real generics, however deeply nested, bounded, or multi-bound, are unaffected. A relational or shift expression whose operands happen to be capitalized, such as `X < Y > Y;`, is not fixed by this and remains ambiguous with a declaration the same way it already was, since capitalization is the only signal available to the lexer at this point and a real type name is conventionally capitalized too.

An untyped parameter in a scripted override of an abstract method generated as `Object`, which does not implement the abstract method's real, narrower signature, so calling it threw `java.lang.AbstractMethodError` instead of running (#812). `abstract class Base { abstract Object run(String s); } Base b = new Base() { run(s) { return "got:" + s; } };` failed this way; `run(String s) { ... }` with an explicit type already worked. Such a method now adopts the abstract method's own parameter types for its untyped positions, when exactly one abstract method of that name and arity can be meant; when two abstract signatures of the same name and arity are both candidates, it's left as `Object`, an `AbstractMethodError` waiting to happen if called, exactly as Java itself requires the script to write two separate typed overrides to disambiguate. Only genuinely abstract methods are ever adopted from, which a scripted method can never legally not override, so this can't reach a `final`, `static`, or `private` method of the same name and arity the way an earlier, broader attempt at this fix once did.

A Java-applicable varargs overload could be preempted by a Java-inapplicable fixed-arity one, for both lambda and ordinary calls (#849). `pick(boolean value, Runnable action)` and `pick(String value, Runnable... actions)` called as `pick("x", () -> {})` picked the `boolean` overload, though `"x"` has no legal conversion to `boolean` at all and real Java only ever reaches the varargs one. The root cause was two-fold: bsh's loose-coercion round didn't recognize an unboxing-then-widening conversion such as `Integer` to `long` (JLS 5.3), so it fell back to bsh's own loosest, Java-inapplicable round to match it — the same round a call with no Java equivalent at all would also only match at, making the two indistinguishable; and every fixed-arity candidate was tried across all of bsh's rounds before any varargs candidate was tried at all, instead of interleaving them the way JLS 15.12.2's three applicability phases do. Both are fixed; overload resolution now recognizes the unbox-then-widen conversion at the same round Java would consider it applicable, and tries fixed-arity and varargs candidates in the same phase order Java does.

A lambda body's ternary expression, such as `true ? 1 : foo()`, was treated as a known constant for overload-resolution narrowing whenever the condition and the taken branch folded, without checking whether the untaken branch was itself constant (#845). JLS 15.28 requires every branch of a conditional expression to fold, not just the one its condition statically selects, so `q(() -> true ? 1 : foo())` picked a `byte`-returning overload where real Java only ever considers the `int`-returning one. The same fix also corrects a previously undetected case in the same lambda-body reachability analysis, where `while (true ? true : foo())` was wrongly treated as an unconditionally infinite loop.

A scripted method called without qualification from inside a block, an `if`/`for`/`while`/`try` body, or a lambda ran inside the caller's own namespace instead of one of its own (#841). Two methods sharing a same-named parameter of different types could not both be called from the same block: `m(Integer c) { } n(String c) { } { m(1); n("s"); }` threw `Typed variable: c was previously declared with type: ...`. The same mechanism let a method's parameters and locals leak into the calling scope and persist after it returned, and let a leaking parameter that shared both name and type with an existing caller variable silently overwrite it, with no error at all. Such a method now gets its own namespace, parented on the caller's, so a no-arg method still late-binds to a variable redeclared in an enclosing block (the behavior #676 added), while its own parameters and locals stay local and the caller's variables are left untouched.
