<!--
	...
	commands.  The output normally shows the file comment and method signatures.
	In the event that there is no file comment the comment for the first 
	method whos name matches the file name is used.
...
<xsl:stylesheet	
	...
	<xsl:template match="File">
		...
		<a name="{$name}">
		<strong><font size="+2">
			<xsl:value-of select="$name"/></font></strong><br/>
		</a>
		<font size="+1">
			<xsl:call-template name="showMethods"/>	
		</font>
