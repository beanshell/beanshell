...
class BSHTryStatement extends SimpleNode {
    ...
    /**
     * Instantiates a new BSH try statement.
     *
     * @param id
     *            the id
     */
    BSHTryStatement(final int id) {
        super(id);
    }
    ...
    public Object eval(final CallStack callstack, final Interpreter interpreter)
            throws EvalError {
        final BSHBlock tryBlock = (BSHBlock) this.jjtGetChild(0);
        final Vector catchParams = new Vector();
        final Vector catchBlocks = new Vector();
        final int nchild = this.jjtGetNumChildren();
        Node node = null;
        int i = 1;
        while (i < nchild && (node = this
                .jjtGetChild(i++)) instanceof BSHFormalParameter) {
         ...
         */
        final int callstackDepth = callstack.depth();
        try {
            ret = tryBlock.eval(callstack, interpreter);
        } catch (final TargetError e) {
            target = e;
            final StringBuilder stackInfo = new StringBuilder("Bsh Stack: ");
            while (callstack.depth() > callstackDepth)
                stackInfo.append("\t" + callstack.pop() + "\n");
        }
        ...
        if (thrown != null) {
            final int n = catchParams.size();
            for (i = 0; i < n; i++) {
                // Get catch block
                final BSHFormalParameter fp = (BSHFormalParameter) catchParams
                        .elementAt(i);
                ...
                if (fp.type == null && interpreter.getStrictJava())
                    throw new EvalError("(Strict Java) Untyped catch block",
                            this, callstack);
                // If the param is typed check assignability
                if (fp.type != null)
                    try {
                        thrown = (Throwable) Types.castObject(thrown/* rsh */,
                                fp.type/* lhsType */, Types.ASSIGNMENT);
                ...
                final BSHBlock cb = (BSHBlock) catchBlocks.elementAt(i);
                ...
                final BlockNameSpace cbNameSpace = new BlockNameSpace(
                        enclosingNameSpace);
                try {
                    if (fp.type == BSHFormalParameter.UNTYPED)
                        ...
                        new Modifiers();
                        cbNameSpace.setTypedVariable(fp.name, fp.type, thrown,
                                new Modifiers()/* none */);
                    }
                } catch (final UtilEvalError e) {
