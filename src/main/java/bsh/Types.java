...
class Types {
     ...
     * smaller
     * types (as in an explicit Java cast) and things allowed only in variable
     * and array
     * declarations (e.g. byte b = 42;)
     */
    static final int CAST = 0, ASSIGNMENT = 1;
    /** The Constant BSH_ASSIGNABLE. */
    static final int JAVA_BASE_ASSIGNABLE = 1, JAVA_BOX_TYPES_ASSIGABLE = 2,
            JAVA_VARARGS_ASSIGNABLE = 3, BSH_ASSIGNABLE = 4;
    /** The Constant LAST_ROUND_ASSIGNABLE. */
     ...
     * @param round
     *            the round
     * @return true, if is signature assignable
     * Should check for strict java here and limit to isJavaAssignable() */
    static boolean isSignatureAssignable(final Class[] from, final Class[] to,
            final int round) {
     ...
     */
    static boolean areSignaturesEqual(final Class[] from, final Class[] to) {
    ...
    /**
     * Checks if is signature varargs assignable.
     *
     * @param from
     *            the from
     * @param to
     *            the to
     * @return true, if is signature varargs assignable
     */
    private static boolean isSignatureVarargsAssignable(final Class[] from,
            final Class[] to) {
        return false;
    }
     ...
     * the
     * value Primitive.NULL and allows it to be assigned to any reference
     * lhsType
     * type (non primitive).
     * <p/>
     *
     ...
     * @param rhsType
     *            assigning from rhsType to lhsType
     * @return true, if is java assignable
     * @see #isBshAssignable(Class, Class)
     */
    static boolean isJavaAssignable(final Class lhsType, final Class rhsType) {
     ...
     * @return true, if is java base assignable
     */
    static boolean isJavaBaseAssignable(final Class lhsType,
            final Class rhsType) {
        /*
        ...
        if (lhsType.isPrimitive() && rhsType.isPrimitive()) {
            ...
            if (rhsType == Byte.TYPE && (lhsType == Short.TYPE
            ...
            if (rhsType == Character.TYPE
                    && (lhsType == Integer.TYPE || lhsType == Long.TYPE
                            || lhsType == Float.TYPE || lhsType == Double.TYPE))
                return true;
            if (rhsType == Integer.TYPE && (lhsType == Long.TYPE
                    || lhsType == Float.TYPE || lhsType == Double.TYPE))
                return true;
     ...
     * @param rhsType
     *            the rhs type
     * @return true, if is java box types assignable
     */
    static boolean isJavaBoxTypesAssignable(final Class lhsType,
            final Class rhsType) {
        // Assignment to loose type... defer to bsh extensions
        if (lhsType == null)
            return false;
        ...
        if (lhsType == Number.class && rhsType != Character.TYPE
     ...
     * @param fromType
     *            the from type
     * @return true, if is bsh assignable
     */
    static boolean isBshAssignable(final Class toType, final Class fromType) {
        try {
            return castObject(toType, fromType, null/* fromValue */, ASSIGNMENT,
                    true/* checkOnly */
            ) == VALID_CAST;
     ...
     * @param toType
     *            the class type of the cast result, which may include
     ...
     * @see #isBshAssignable(Class, Class)
     */
    public static Object castObject(final Object fromValue, final Class toType,
            final int operation) throws UtilEvalError {
        if (fromValue == null)
            throw new InterpreterError("null fromValue");
        final Class fromType = fromValue instanceof Primitive
                ? ((Primitive) fromValue).getType()
                : fromValue.getClass();
        return castObject(toType, fromType, fromValue, operation,
                false/* checkonly */);
    }
     ...
     * to
     * perform primtive operations that cast to a smaller type. e.g.
     * (byte)myLong;
     * These are used in explicit primitive casts, primitive delclarations and
     * array declarations. I don't believe there are any object conversions
     * which are
     * different between ASSIGNMENT and CAST (e.g. scripted object to interface
     * proxy
     ...
     * @param toType
     *            the class type of the cast result, which may include
     *            primitive types, e.g. Byte.TYPE. toType may be null to
     *            indicate a
     *            loose type assignment (which matches any fromType).
     * @param fromType
     *            is the class type of the value to be cast including
     *            java primitive TYPE classes for primitives.
     *            If fromValue is (or would be) Primitive.NULL then fromType
     *            should be null.
     * @param fromValue
     *            an Object or bsh.Primitive primitive value (including
     *            Primitive.NULL or Primitive.VOID)
     ...
     * @param checkOnly
     *            If checkOnly is true then fromValue must be null.
     ...
     * @return the object
     * @throws UtilEvalError
     *             on invalid assignment (when operation is
     *             assignment).
     ...
     */
    private static Object castObject(final Class toType, final Class fromType,
            final Object fromValue, final int operation,
            final boolean checkOnly) throws UtilEvalError {
        /*
        ...
        if (toType.isPrimitive())
            if (fromType == Void.TYPE || fromType == null
                ...
                return Primitive.castPrimitive(toType, fromType,
                        (Primitive) fromValue, checkOnly, operation);
            else if (Primitive.isWrapperType(fromType)) {
                // wrapper to primitive
                // Convert value to Primitive and check/cast it.
                // Object r = checkOnly ? VALID_CAST :
                final Class unboxedFromType = Primitive.unboxType(fromType);
                Primitive primFromValue;
                if (checkOnly)
                    primFromValue = null; // must be null in checkOnly
                else
                    primFromValue = (Primitive) Primitive.wrap(fromValue,
                            unboxedFromType);
                return Primitive.castPrimitive(toType, unboxedFromType,
                        primFromValue, checkOnly, operation);
        ...
        if (fromType == Void.TYPE || fromType == null
            ...
            if (Primitive.isWrapperType(toType) && fromType != Void.TYPE
                    && fromType != null)
                // primitive to wrapper type
                return checkOnly ? VALID_CAST
                        : Primitive.castWrapper(Primitive.unboxType(toType),
                                ((Primitive) fromValue).getValue());
            // Primitive (not null or void) to Object.class type
            if (toType == Object.class && fromType != Void.TYPE
                    && fromType != null)
                // box it
                return checkOnly ? VALID_CAST
                        : ((Primitive) fromValue).getValue();
     ...
     */
    static UtilEvalError castError(final Class lhsType, final Class rhsType,
            final int operation) {
        return castError(Reflect.normalizeClassName(lhsType),
     ...
     * @return the util eval error
     */
    static UtilEvalError castError(final String lhs, final String rhs,
            final int operation) {
        if (operation == ASSIGNMENT)
            return new UtilEvalError("Can't assign " + rhs + " to " + lhs);
        final Exception cce = new ClassCastException(
