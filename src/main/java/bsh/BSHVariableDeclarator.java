...
class BSHVariableDeclarator extends SimpleNode {
    ...
    /**
     * Instantiates a new BSH variable declarator.
     *
     * @param id
     *            the id
     */
    BSHVariableDeclarator(final int id) {
        super(id);
    }
     ...
     *            int [] a = { 1, 2 };
     *            typeNode may be null to indicate no type information
     *            available.
     * @param callstack
     *            the callstack
     * @param interpreter
     ...
     *             the eval error
     */
    public Object eval(final BSHType typeNode, final CallStack callstack,
            final Interpreter interpreter) throws EvalError {
        // null value means no value
        Object value = null;
        if (this.jjtGetNumChildren() > 0) {
            final SimpleNode initializer = (SimpleNode) this.jjtGetChild(0);
            /*
