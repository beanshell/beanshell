...
public class PreparsedScript {
	...
	public PreparsedScript(final String source, final ClassLoader classLoader) throws EvalError {
		...
		final NameSpace nameSpace = new NameSpace((BshClassManager)classManager, "global");
	...
	public Object invoke(final Map<String, ?> context) throws EvalError {
		final NameSpace nameSpace = new NameSpace(_interpreter.getNameSpace(), "BeanshellExecutable");
		...
		for (final Map.Entry<String, ?> entry : context.entrySet()) {
			try {
				...
				throw new IllegalStateException("cannot set variable '" + entry.getKey() + '\'', e);
			}
		}
