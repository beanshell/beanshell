...
/**
 * The Class BSHPrimaryExpression.
 */
class BSHPrimaryExpression extends SimpleNode {
    ...
    /**
     * Instantiates a new BSH primary expression.
     *
     * @param id
     *            the id
     */
    BSHPrimaryExpression(final int id) {
        super(id);
    }
    ...
    public LHS toLHS(final CallStack callstack, final Interpreter interpreter)
            throws EvalError {
        final Object obj = this.eval(true, callstack, interpreter);
     ...
     *             the eval error
     */
    private Object eval(final boolean toLHS, final CallStack callstack,
        ...
        for (int i = 1; i < numChildren; i++)
            obj = ((BSHPrimarySuffix) this.jjtGetChild(i)).doSuffix(obj, toLHS,
        ...
        if (obj instanceof SimpleNode)
            if (obj instanceof BSHAmbiguousName)
                if (toLHS)
                    obj = ((BSHAmbiguousName) obj).toLHS(callstack,
                            interpreter);
                else
                    obj = ((BSHAmbiguousName) obj).toObject(callstack,
                            interpreter);
            else
