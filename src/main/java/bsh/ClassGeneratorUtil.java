 ...
 *
 *         Notes:
 *         It would not be hard to eliminate the use of org.objectweb.asm.Type
 *         from
...
public class ClassGeneratorUtil implements Constants {
    ...
    /**
     * Instantiates a new class generator util.
     *
     ...
     *            the is interface
     */
    public ClassGeneratorUtil(final Modifiers classModifiers,
            final String className, final String packageName, Class superClass,
            Class[] interfaces, final Variable[] vars,
            final DelayedEvalBshMethod[] bshmethods,
        ...
        this.splitMethodsAndConstructors(className, bshmethods);
    }
     ...
     * @param instanceInitBlock
     *            the instance init block
     */
    public void initStaticNameSpace(final NameSpace classStaticNameSpace,
            final BSHBlock instanceInitBlock) {
        try {
            classStaticNameSpace.setLocalVariable(BSHCONSTRUCTORS,
                    this.constructors, false/* strict */);
            classStaticNameSpace.setLocalVariable(BSHINIT, instanceInitBlock,
                    false/* strict */);
     ...
     *            the bshmethods
     */
    private void splitMethodsAndConstructors(final String className,
            final DelayedEvalBshMethod[] bshmethods) {
        // Split the methods into constructors and regular method lists
        final List consl = new ArrayList();
        final List methodsl = new ArrayList();
        final String classBaseName = getBaseName(className); // for inner
                                                             // classes
        for (final DelayedEvalBshMethod bm : bshmethods)
            if (bm.getName().equals(classBaseName)
                    && bm.getReturnType() == null)
                consl.add(bm);
            else
                methodsl.add(bm);
        this.constructors = (DelayedEvalBshMethod[]) consl
                .toArray(new DelayedEvalBshMethod[0]);
        this.methods = (DelayedEvalBshMethod[]) methodsl
                .toArray(new DelayedEvalBshMethod[0]);
    }
     ...
     *
     * @param generateInitCode
     *            the generate init code
     * @return the byte[]
     */
    public byte[] generateClass(final boolean generateInitCode) {
        // Force the class public for now...
        int classMods = getASMModifiers(this.classModifiers) | ACC_PUBLIC;
        if (this.isInterface)
            classMods |= ACC_INTERFACE;
        final String[] interfaceNames = new String[this.interfaces.length];
        for (int i = 0; i < this.interfaces.length; i++)
            interfaceNames[i] = Type.getInternalName(this.interfaces[i]);
        final String sourceFile = "BeanShell Generated via ASM (www.objectweb.org)";
        final ClassWriter cw = new ClassWriter(true);
        cw.visit(classMods, this.fqClassName, this.superClassName,
                interfaceNames, sourceFile);
        if (!this.isInterface) {
            // Generate the bsh instance 'This' reference holder field
            generateField(BSHTHIS + this.className, "Lbsh/This;", ACC_PUBLIC,
                    cw);
            // Generate the static bsh static reference holder field
            generateField(BSHSTATIC + this.className, "Lbsh/This;",
                    ACC_PUBLIC + ACC_STATIC, cw);
        }
        // Generate the fields
        for (final Variable var : this.vars) {
            final String type = var.getTypeDescriptor();
        ...
        for (int i = 0; i < this.constructors.length; i++) {
            ...
            if (this.constructors[i].hasModifier("private"))
                continue;
            final int modifiers = getASMModifiers(
                    this.constructors[i].getModifiers());
            this.generateConstructor(i,
                    this.constructors[i].getParamTypeDescriptors(), modifiers,
                    cw);
            hasConstructor = true;
        }
        // If no other constructors, generate a default constructor
        if (!this.isInterface && !hasConstructor)
            this.generateConstructor(DEFAULTCONSTRUCTOR/* index */,
                    new String[0], ACC_PUBLIC, cw);
        // Generate the delegate methods
        for (final DelayedEvalBshMethod method : this.methods) {
            final String returnType = method.getReturnTypeDescriptor();
            ...
            generateMethod(this.className, this.fqClassName, method.getName(),
                    returnType, method.getParamTypeDescriptors(), modifiers,
                    cw);
            final boolean isStatic = (modifiers & ACC_STATIC) > 0;
            final boolean overridden = classContainsMethod(this.superClass,
                    method.getName(), method.getParamTypeDescriptors());
            if (!isStatic && overridden)
                generateSuperDelegateMethod(this.superClassName,
     ...
     * @param modifiers
     *            the modifiers
     * @return the ASM modifiers
     */
    static int getASMModifiers(final Modifiers modifiers) {
        int mods = 0;
        if (modifiers == null)
            return mods;
     ...
     *            the cw
     */
    static void generateField(final String fieldName, final String type,
            final int modifiers, final ClassWriter cw) {
     ...
     *            the cw
     */
    static void generateMethod(final String className, final String fqClassName,
            final String methodName, String returnType,
            final String[] paramTypes, final int modifiers,
            final ClassWriter cw) {
        final String[] exceptions = null;
        final boolean isStatic = (modifiers & ACC_STATIC) != 0;
        if (returnType == null) // map loose return type to Object
            returnType = OBJECT;
        final String methodDescriptor = getMethodDescriptor(returnType,
                paramTypes);
        // Generate method body
        final CodeVisitor cv = cw.visitMethod(modifiers, methodName,
        ...
        if (isStatic)
            ...
            cv.visitFieldInsn(GETFIELD, fqClassName, BSHTHIS + className,
                    "Lbsh/This;");
        }
        ...
        cv.visitMethodInsn(INVOKEVIRTUAL, "bsh/This", "invokeMethod",
        ...
        cv.visitMethodInsn(INVOKESTATIC, "bsh/Primitive", "unwrap",
     ...
     *            the cw
     */
    void generateConstructor(final int index, final String[] paramTypes,
            final int modifiers, final ClassWriter cw) {
        ...
        final String[] exceptions = null;
        final String methodDescriptor = getMethodDescriptor("V", paramTypes);
        // Create this constructor method
        final CodeVisitor cv = cw.visitMethod(modifiers, "<init>",
                methodDescriptor, exceptions);
        // Generate code to push arguments as an object array
        ...
        cv.visitMethodInsn(INVOKESTATIC, "bsh/ClassGeneratorUtil",
                "initInstance",
     ...
     *            the cw
     */
    void generateStaticInitializer(final ClassWriter cw) {
        final CodeVisitor cv = cw.visitMethod(ACC_STATIC, "<clinit>", "()V",
                null/* exceptions */);
        // Generate code to invoke the ClassGeneratorUtil initStatic() method
        // Push the class name as a constant
        cv.visitLdcInsn(this.fqClassName);
        ...
        cv.visitMethodInsn(INVOKESTATIC, "bsh/ClassGeneratorUtil", "initStatic",
                "(Ljava/lang/Class;)V");
        cv.visitInsn(RETURN);
        // values here are ignored, computed automatically by ClassWriter
        cv.visitMaxs(0, 0);
     ...
     *            the cv
     */
    void generateConstructorSwitch(final int consIndex, final int argsVar,
            final int consArgsVar, final CodeVisitor cv) {
        final Label defaultLabel = new Label();
        final Label endLabel = new Label();
        final int cases = this.superConstructors.length
        ...
        cv.visitLdcInsn(this.superClass.getName()); // use superClassName var?
        // Push the bsh static namespace field
        pushBshStatic(this.fqClassName, this.className, cv);
        // push args
        cv.visitVarInsn(ALOAD, argsVar);
        ...
        cv.visitFieldInsn(GETFIELD, "bsh/ClassGeneratorUtil$ConstructorArgs",
                "selector", "I");
        // start switch
        cv.visitTableSwitchInsn(0/* min */, cases - 1/* max */, defaultLabel,
                labels);
        // generate switch body
        int index = 0;
        for (int i = 0; i < this.superConstructors.length; i++, index++)
            doSwitchBranch(index, this.superClassName,
                    getTypeDescriptors(
                            this.superConstructors[i].getParameterTypes()),
                    endLabel, labels, consArgsVar, cv);
        for (int i = 0; i < this.constructors.length; i++, index++)
            doSwitchBranch(index, this.fqClassName,
                    this.constructors[i].getParamTypeDescriptors(), endLabel,
                    labels, consArgsVar, cv);
        // generate the default branch of switch
        cv.visitLabel(defaultLabel);
        // default branch always invokes no args super
        cv.visitVarInsn(ALOAD, 0); // push 'this'
        cv.visitMethodInsn(INVOKESPECIAL, this.superClassName, "<init>", "()V");
        // done with switch
    ...
    private static void pushBshStatic(final String fqClassName,
            final String className, final CodeVisitor cv) {
        cv.visitFieldInsn(GETSTATIC, fqClassName, BSHSTATIC + className,
                "Lbsh/This;");
    }
     ...
     * on the stack.
     */
    static void doSwitchBranch(final int index, final String targetClassName,
            final String[] paramTypes, final Label endLabel,
            final Label[] labels, final int consArgsVar, final CodeVisitor cv) {
        ...
        for (final String type : paramTypes) {
            ...
            final String className = "bsh/ClassGeneratorUtil$ConstructorArgs";
            String retType;
            if (method.equals("getObject"))
                retType = OBJECT;
            else
                retType = type;
            cv.visitMethodInsn(INVOKEVIRTUAL, className, method,
                    "()" + retType);
        ...
        final String descriptor = getMethodDescriptor("V", paramTypes);
        cv.visitMethodInsn(INVOKESPECIAL, targetClassName, "<init>",
                descriptor);
     ...
     */
    static String getMethodDescriptor(final String returnType,
    ...
    static void generateSuperDelegateMethod(final String superClassName,
            final String methodName, String returnType,
            final String[] paramTypes, final int modifiers,
            final ClassWriter cw) {
        final String[] exceptions = null;
        ...
        final String methodDescriptor = getMethodDescriptor(returnType,
                paramTypes);
        // Add method body
        final CodeVisitor cv = cw.visitMethod(modifiers,
                "_bshSuper" + methodName, methodDescriptor, exceptions);
        cv.visitVarInsn(ALOAD, 0);
        // Push vars
        int localVarIndex = 1;
        for (final String paramType : paramTypes) {
            ...
            localVarIndex += paramType.equals("D") || paramType.equals("J") ? 2
                    : 1;
        }
        cv.visitMethodInsn(INVOKESPECIAL, superClassName, methodName,
                methodDescriptor);
     ...
     */
    static boolean classContainsMethod(Class clas, final String methodName,
            final String[] paramTypes) {
        while (clas != null) {
            final Method[] methods = clas.getDeclaredMethods();
            for (final Method method : methods)
                if (method.getName().equals(methodName)) {
                    final String[] methodParamTypes = getTypeDescriptors(
     ...
     *            the cv
     */
    static void generatePlainReturnCode(final String returnType,
     ...
     */
    public static void generateParameterReifierCode(final String[] paramTypes,
            final boolean isStatic, final CodeVisitor cv) {
        cv.visitIntInsn(SIPUSH, paramTypes.length);
        cv.visitTypeInsn(ANEWARRAY, "java/lang/Object");
        int localVarIndex = isStatic ? 0 : 1;
        for (int i = 0; i < paramTypes.length; ++i) {
            final String param = paramTypes[i];
            cv.visitInsn(DUP);
     ...
     *            the code visitor to be used to generate the bytecode.
     */
    public static void generateReturnCode(final String returnType,
            final CodeVisitor cv) {
        if (returnType.equals("V")) {
            cv.visitInsn(POP);
            cv.visitInsn(RETURN);
     ...
     *            the class static this
     * @param consArgs
     *            the arguments to the constructor. These are necessary in
     *            the evaluation of the alt constructor args. e.g. Foo(a) {
     *            super(a); }
     * @param index
     *            the index
    ...
    public static ConstructorArgs getConstructorArgs(
            final String superClassName, final This classStaticThis,
            final Object[] consArgs, final int index) {
        if (classStaticThis == null)
            throw new InterpreterError("Unititialized class: no static");
        DelayedEvalBshMethod[] constructors;
        try {
            final Object cons = classStaticThis.getNameSpace()
        ...
        final DelayedEvalBshMethod constructor = constructors[index];
        ...
        SimpleNode firstStatement = (SimpleNode) constructor.methodBody
                .jjtGetChild(0);
        ...
        if (firstStatement instanceof BSHMethodInvocation) {
            final BSHMethodInvocation methodNode = (BSHMethodInvocation) firstStatement;
            final BSHAmbiguousName methodName = methodNode.getNameNode();
        ...
        final NameSpace consArgsNameSpace = new NameSpace(
                classStaticThis.getNameSpace(), "consArgs");
        final String[] consArgNames = constructor.getParameterNames();
        final Class[] consArgTypes = constructor.getParameterTypes();
        for (int i = 0; i < consArgs.length; i++)
            try {
                consArgsNameSpace.setTypedVariable(consArgNames[i],
                        consArgTypes[i], consArgs[i], null/* modifiers */);
            } catch (final UtilEvalError e) {
        ...
        Object[] args = null;
        final Interpreter interpreter = classStaticThis.declaringInterpreter;
        try {
        ...
        final Class superClass = interpreter.getClassManager()
                .classForName(superClassName);
        if (superClass == null)
            throw new InterpreterError(
                    "can't find superclass: " + superClassName);
        final Constructor[] superCons = superClass.getDeclaredConstructors();
        // find the matching super() constructor for the args
        if (altConstructor.equals("super")) {
            final int i = Reflect.findMostSpecificConstructorIndex(argTypes,
                    superCons);
            if (i == -1)
        ...
        final int i = Reflect.findMostSpecificSignature(argTypes, candidates);
        if (i == -1)
            throw new InterpreterError("can't find constructor for args 2!");
        // this() constructors come after super constructors in the table
        final int selector = i + superCons.length;
        final int ourSelector = index + superCons.length;
    ...
    public static void initInstance(final Object instance,
            final String className, final Object[] args) {
        final Class[] sig = Types.getTypes(args);
        final CallStack callstack = new CallStack();
        ...
        if (instanceThis == null) {
            ...
            final This classStaticThis = getClassStaticThis(instance.getClass(),
                    className);
            if (classStaticThis == null)
                throw new InterpreterError(
                        "Failed to init class: " + className);
            ...
            instanceNameSpace = new NameSpace(classStaticThis.getNameSpace(),
                    className);
            ...
            try {
                final LHS lhs = Reflect.getLHSObjectField(instance,
                        BSHTHIS + className);
            ...
            try { // Evaluate the initializer block
                instanceInitBlock.evalBlock(callstack, interpreter,
                        true/* override */,
                        ClassGeneratorImpl.ClassNodeFilter.CLASSINSTANCE);
            } catch (final Exception e) {
                throw new InterpreterError(
                        "Error in class initialization: " + e);
            }
            callstack.pop();
        } else {
        ...
        final String constructorName = getBaseName(className);
        try {
            // Find the constructor (now in the instance namespace)
            BshMethod constructor = instanceNameSpace.getMethod(constructorName,
                    sig, true/* declaredOnly */);
     ...
     *            the gen class
     */
    public static void initStatic(final Class genClass) {
     ...
     * @return the class static this
     */
    static This getClassStaticThis(final Class clas, final String className) {
        try {
            return (This) Reflect.getStaticFieldValue(clas,
                    BSHSTATIC + className);
        } catch (final Exception e) {
            throw new InterpreterError(
                    "Unable to get class static space: " + e);
        }
    }
     ...
     *         been initialized.
     */
    static This getClassInstanceThis(final Object instance,
            final String className) {
        try {
            final Object o = Reflect.getObjectFieldValue(instance,
                    BSHTHIS + className);
     ...
     */
    private static boolean isPrimitive(final String typeDescriptor) {
     ...
     */
    static String[] getTypeDescriptors(final Class[] cparams) {
        final String[] sa = new String[cparams.length];
    ...
    private static String descriptorToClassName(final String s) {
     ...
     */
    private static String getBaseName(final String className) {
    ...
    public static class ConstructorArgs {
        ...
        /**
         * Instantiates a new constructor args.
         *
         * @param selector
         *            the selector
         * @param args
         *            the args
         */
        ConstructorArgs(final int selector, final Object[] args) {
     ...
     *            the gen class
     */
    public static void startInterpreterForClass(final Class genClass) {
        final String fqClassName = genClass.getName();
        final String baseName = Name.suffix(fqClassName, 1);
        final String resName = baseName + ".bsh";
        final InputStream in = genClass.getResourceAsStream(resName);
        if (in == null)
            throw new InterpreterError(
        ...
        final NameSpace globalNS = bsh.getNameSpace();
