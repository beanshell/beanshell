...
class BSHSwitchStatement extends SimpleNode implements ParserConstants {
    ...
    /**
     * Instantiates a new BSH switch statement.
     *
     * @param id
     *            the id
     */
    public BSHSwitchStatement(final int id) {
        super(id);
    }
    ...
    public Object eval(final CallStack callstack, final Interpreter interpreter)
        ...
        int child = 0;
        final SimpleNode switchExp = (SimpleNode) this.jjtGetChild(child++);
        ...
        label = (BSHSwitchLabel) this.jjtGetChild(child++);
        ...
        while (child < numchild && returnControl == null)
            ...
            if (label.isDefault || this.primitiveEquals(switchVal,
                    label.eval(callstack, interpreter), callstack, switchExp))
                // execute nodes, skipping labels, until a break or return
                while (child < numchild) {
                    node = this.jjtGetChild(child++);
                    if (node instanceof BSHSwitchLabel)
                        continue;
                    // eval it
                    final Object value = ((SimpleNode) node).eval(callstack,
                            interpreter);
     ...
     *             the eval error
     */
    private boolean primitiveEquals(final Object switchVal,
            final Object targetVal, final CallStack callstack,
            final SimpleNode switchExp) throws EvalError {
        if (switchVal instanceof Primitive || targetVal instanceof Primitive)
            try {
                // binaryOperation can return Primitive or wrapper type
                Object result = Primitive.binaryOperation(switchVal, targetVal,
                        ParserConstants.EQ);
                result = Primitive.unwrap(result);
                return result.equals(Boolean.TRUE);
            } catch (final UtilEvalError e) {
                throw e.toEvalError(
                        "Switch value: " + switchExp.getText() + ": ", this,
                        callstack);
            }
        else
