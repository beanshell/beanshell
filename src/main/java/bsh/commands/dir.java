...
public class dir {
     ...
     *            the dir
     */
    public static void invoke(final Interpreter env, final CallStack callstack,
            final String dir) {
        File file;
        String path;
        try {
        ...
        for (final String file2 : files) {
            final File f = new File(path + File.separator + file2);
            ...
            final int fieldlen = 8;
            final StringBuffer len = new StringBuffer();
            for (int j = 0; j < fieldlen; j++)
                len.append(" ");
            len.insert(0, f.length());
            len.setLength(fieldlen);
            // hack to move the spaces to the front
            final int si = len.toString().indexOf(" ");
            if (si != -1) {
                final String pad = len.toString().substring(si);
                len.setLength(si);
                len.insert(0, pad);
            }
