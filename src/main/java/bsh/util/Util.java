...
public class Util {
    ...
    public static void startSplashScreen() {
        final int width = 275, height = 148;
        final Window win = new Window(new Frame());
        win.pack();
        final BshCanvas can = new BshCanvas();
        can.setSize(width, height); // why is this necessary?
        final Toolkit tk = Toolkit.getDefaultToolkit();
        final Dimension dim = tk.getScreenSize();
        win.setBounds(dim.width / 2 - width / 2, dim.height / 2 - height / 2,
                width, height);
        win.add("Center", can);
        final Image img = tk.getImage(
                Interpreter.class.getResource("/bsh/util/lib/splash.gif"));
        final MediaTracker mt = new MediaTracker(can);
        mt.addImage(img, 0);
        try {
            mt.waitForAll();
        } catch (final Exception e) {}
        final Graphics gr = can.getBufferedGraphics();
